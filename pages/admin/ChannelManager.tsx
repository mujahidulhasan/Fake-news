...
export const ChannelManager: React.FC = () => {
  ...
  const handleEdit = (channel: Channel) => {
    ...
    setIsPinned(channel.isPinned || false);
  };

  const handleCancel = () => {
    setEditingId(null);
    setName(''); setSlug(''); setLogoUrl(''); setDescription(''); setCategory('General'); setIsPinned(false);
  };
        ...
        <div className="md:col-span-1">
            <GlassCard>
                ...
                <form onSubmit={handleSave} className="space-y-3">
                    ...
                    <div>
                         <label className="block text-xs font-bold text-gray-500 mb-1">Category (Section)</label>
                         <input type="text" list="categories" placeholder="e.g. Breaking News" value={category} onChange={e => setCategory(e.target.value)} className="w-full p-2 border rounded bg-white/50 text-sm" />
                         <datalist id="categories">
                             <option value="General" />
                             <option value="Breaking News" />
                             <option value="Facebook Fake Post" />
                             <option value="Sports" />
                         </datalist>
                    </div>
                    <div className="flex items-center gap-2">
                        <input type="checkbox" id="pinned" checked={isPinned} onChange={e => setIsPinned(e.target.checked)} className="w-4 h-4" />
                        <label htmlFor="pinned" className="text-xs font-bold text-gray-700">Pin to Top of Section</label>
                    </div>
            ...
            {channels.map(channel => (
                <div key={channel._id} className="bg-white p-3 rounded-lg shadow-sm border border-gray-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <span className="font-bold text-gray-800 text-sm block flex items-center gap-2">
                                {channel.name}
                                {channel.isPinned && <span className="text-[10px] bg-blue-100 text-blue-600 px-1 rounded flex items-center"><Icons.Pin/> Pinned</span>}
                            </span>
                            <span className="text-xs text-gray-400 bg-gray-100 px-1.5 rounded">{channel.category}</span>
                        </div>
                    </div>
                    <div className="flex gap-2">
